...
class ReconcileResult:
    ...
    def resolution_summary(self) -> dict[str, int]:
        """Count decisions by resolution type: merge, relate, no_match, review."""
        counts: dict[str, int] = {"merge": 0, "relate": 0, "no_match": 0, "review": 0}
        for d in self.decisions:
            res = d.get("resolution")
            if res is None:
                # Backwards compat: derive from decision field
                dec = d.get("decision", "")
                if dec == "merge":
                    counts["merge"] += 1
                elif dec == "review":
                ...
                key = res if res in counts else "no_match"
                counts[key] += 1
            elif isinstance(res, dict) and "relate" in res:
                counts["relate"] += 1
        return counts

    @property
    def relationships(self) -> list[dict[str, Any]]:
        """Extract all Relate decisions with link_type, direction, and subtype info."""
        rels: list[dict[str, Any]] = []
        for d in self.decisions:
            res = d.get("resolution")
            if isinstance(res, dict) and "relate" in res:
                rel: dict[str, Any] = {
                    "entity_a_id": d["entity_a_id"],
                    "entity_b_id": d["entity_b_id"],
                    "confidence": d["confidence"],
                    "link_type": res["relate"]["link_type"],
                    "direction": res["relate"].get("direction", "symmetric"),
                }
                if d.get("subtype_a"):
                    rel["subtype_a"] = d["subtype_a"]["subtype"]
                if d.get("subtype_b"):
                    rel["subtype_b"] = d["subtype_b"]["subtype"]
                rels.append(rel)
        return rels

    def to_resolution_graph(self) -> Any:
        """Build a networkx graph from clusters (merge) and relationships (relate).

        Nodes are canonical entities (from golden records).
        Edges are typed relationship links (variant_of, sku_of, etc).
        ...
        try:
            ...
            raise ImportError(
                "to_resolution_graph() requires networkx: pip install networkx"
            ) from None

        G = nx.DiGraph()

        # Add cluster nodes (merged entities)
        for i, cluster in enumerate(self.clusters):
            kid = (
                self.golden_records[i].get("kanoniv_id", f"cluster_{i}")
                if i < len(self.golden_records)
                else f"cluster_{i}"
            )
            golden = dict(self.golden_records[i]) if i < len(self.golden_records) else {}
            golden["type"] = "entity"
            golden["member_count"] = str(len(cluster))
            G.add_node(kid, **golden)

        # Map entity UUIDs to their cluster's kanoniv_id
        uuid_to_kid: dict[str, str] = {}
        for i, cluster in enumerate(self.clusters):
            kid = (
                self.golden_records[i].get("kanoniv_id", f"cluster_{i}")
        ...
        for rel in self.relationships:
            src = uuid_to_kid.get(rel["entity_a_id"])
            dst = uuid_to_kid.get(rel["entity_b_id"])
